...
|-----------------------|--------|---------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| /mp                   | POST   | mp - message protocol, required msgType - Eiffel event type, required failIfMultipleFound - default: false failIfNoneFound - default: false, connectToExternalERs - default: true, limit - default: 1| { "msgParams": {"meta": {# Matches the meta object }},"eventParams": {"data": {# Matches the data object},"links": { # Matches the links object } }}   | This endpoint is used to generate Eiffel events and then the obtained event could  be published by [Eiffel REMReM Publish](https://github.com/eiffel-community/eiffel-remrem-publish). |
...
| connectToExternalERs:             | True          | This parameter is to control the behavior of ER towards external ERs,default is true which states the ER queries can be connected to internal ER.else the ER queries can be connected to external ERs.|
| limit:            | 1             | This property is similar to limit in mongoDB. You can specify a value greater than 0, and can say how many events you want default 1. |